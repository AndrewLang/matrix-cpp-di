...
shared_ptr<ServiceDescriptor> ServiceContainer::findDescriptor(string typeName, string instanceName)
{
    ...
    {
        if ((Strings::isNullOrEmpty(instanceName) && descriptor->serviceType->getName() == typeName) ||
            (!Strings::isNullOrEmpty(instanceName) && descriptor->serviceType->getName() == typeName && descriptor->alias == instanceName))
        {
            result = descriptor;
            break;
        }
...
shared_ptr<InstanceDescriptor> ServiceContainer::findInstance(const shared_ptr<TypeInfo> type, string instanceName)
{
    return Vectors::firstOrDefault<shared_ptr<InstanceDescriptor>>(mInstances, [=](shared_ptr<InstanceDescriptor> descriptor) {
        return Strings::isNullOrEmpty(instanceName) ? descriptor->type->getName() == type->getName()
                                                    : descriptor->type->getName() == type->getName() && descriptor->alias == instanceName;
    });
}
} // namespace Matrix
