...
namespace Matrix
{
	...
	shared_ptr<IApplicationStartBlock> Connectivity::ApplicationStartBlock::withName(std::string blockName)
	{
		name = blockName;
		initLogger(nullptr);
	...
	shared_ptr<IApplicationStartBlock> Connectivity::ApplicationStartBlock::withOrder(int blockOrder)
	{
		order = blockOrder;
		return shared_from_this();
	}

	shared_ptr<IApplicationStartBlock> Connectivity::ApplicationStartBlock::withServices(shared_ptr<IServiceProvider> services)
	{
